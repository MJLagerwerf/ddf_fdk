...
coverage: ## check code coverage quickly with the default Python
	coverage run --source ddf_fdk setup.py test
	coverage report -m
	coverage html
...
docs: docs/.nojekyll install_dev ## generate Sphinx HTML documentation, including API docs
	rm -f doc_sources/ddf_fdk.rst
	rm -f doc_sources/modules.rst
	sphinx-apidoc -o doc_sources/ ddf_fdk
...
install: clean ## install the package to the active Python's site-packages
    conda install -c astra-toolbox/label/dev astra-toolbox cudatoolkit=9.0
